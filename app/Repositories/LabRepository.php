        ...
        return response()->json([
            ...
            'data' => collect($labRecords->items())->map(function($lab) {
                ...
                if($lab->StartTime > now())
                    $status = "<span class='badge badge-danger'>UpComming</span>" ;
                    ...
                    // Count submissions
                    $submittedCount = AssignmentInfo::where('assingment_id', $lab->assignment_id)
                    ...
                    $precatage = '<div class="progress progress-sm"><div class="progress-bar bg-green" role="progressbar" aria-valuenow="'.round($submissionPercentage, 2) . '" aria-valuemin="0" aria-valuemax="100" style="width: '.round($submissionPercentage, 2).'%"></div></div><small>'.round($submissionPercentage, 2).'% Uploaded</small>';
                ...
                return [
                    ...
                    'status' => $status,
                    'submission_percentage' => $precatage,
                ];
            }),
        ]);        
    }
