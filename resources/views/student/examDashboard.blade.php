            ...
            <div class="col-md-12 mt-5">
                ...
                <div class="card shadow-lg border-0 rounded-lg">
                    <div class="card-header bg-danger text-white">
                        <h3 class="card-title">Exam Title: <strong>{{ $exam->title }}</strong></h3>
                    </div>
                    <div class="card-body">
                        <div>
                            <h6 class="font-weight-bold">Exam info</h6>
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            <div>
                                <h5 class="font-weight-bold">Exam Description</h5>
                                <p class="text-muted">{!! $exam->dec !!}</p>
                            </div>
                        </div>
                        
                        @if ($exam->file_path)
                            <a href="{{ url(Storage::url($exam->file_path)) }}" target="_blank" class="btn btn-light mb-3">
                                <i class="fas fa-download"></i> Download File
                            </a>
                        @endif
                        @if(!empty($examinfo))
                            <div class="alert alert-info" style="font-size: 12px">Your exam response already has been submitted. If you wish to update it, you can upload a new file, which will replace the existing one.</div>
                        @endif
                        <!-- Dropzone for File Upload -->
                        <form action="{{ route('student.exam.submit') }}" method="POST" enctype="multipart/form-data" class="form" id="exam-form">
                            @csrf
                            <div class="fallback">
